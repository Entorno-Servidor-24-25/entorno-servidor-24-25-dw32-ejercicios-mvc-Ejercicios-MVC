...
Al cargar index.php lo primero que hace es definir en la constante BASE_PATH la ruta donde se está ejecutando el fichero (var/www/html/src/). Luego, carga el fichero UserController.php  mediante la ruta base anteriormente creada y la carpeta donde se encuentra (y el propio fichero). Require es similar a include, solo que al no encontrar el fichero indicado include da un warning y require da error. También, el _once hace que el fichero se incluya una vez, es decir, si el mismo fichero se incluye en un fichero más adelante, lo reutilizará. Por último, define un objeto UserController y llama a la función showForm().
